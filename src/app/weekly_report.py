...
def week_metrics(df, end):
    ...
    return dict(start=start, end=end, w=w, nc=nc, filed=filed,
                gmv=gmv, orders=orders, aov=gmv / orders if orders else 0,
                placed=len(w), cancelled=int((w.order_status == "cancelled").sum()),
...
def delta_span(d, invert=False, label="vs prev. week", size=12, tip=""):
    ...
    return (f"<span class='{cls}' style='font-size:{size}px'{t}>"
...
def spark(vals, w=120, h=30):
    ...
    rng = (vmax - vmin) or 1
    pts = [(i * (w - 8) / (len(vals) - 1) + 4, h - 5 - (v - vmin) / rng * (h - 12))
...
def vbars_grouped(labels, prev, cur, w=780, h=210, fmt=money):
    ...
    n = len(labels); slot = (w - 40) / n; bw = slot * 0.32
    bars = ""
    for i, lb in enumerate(labels):
        x0 = 30 + i * slot + slot * 0.16
        for j, (v, col, tag) in enumerate([(prev[i], PREVBAR, "previous week"),
                                           (cur[i],  NAVY,    "this week")]):
            bh = v / top * (h - 55)
            bars += (f"<rect x='{x0 + j*bw:.1f}' y='{h-30-bh:.1f}' width='{bw:.1f}' height='{bh:.1f}' "
                     f"rx='2' fill='{col}'><title>{lb} · {tag}: {fmt(v)}</title></rect>")
        bars += (f"<text x='{30 + i*slot + slot/2:.1f}' y='{h-12}' text-anchor='middle' "
                 f"class='ax'>{lb}</text>")
    legend = (f"<rect x='30' y='6' width='10' height='10' rx='2' fill='{PREVBAR}'/>"
...
def donut(items, total, fmt=money, center_label="GMV"):
    ...
    for i, (name, v) in enumerate(items):
        ...
        segs += (f"<circle class='seg' cx='85' cy='85' r='58' fill='none' stroke='{col}' "
        ...
        cum += share
    svg = (f"<svg viewBox='0 0 170 170' width='170' height='170'>{segs}"
...
def stacked_bar(items, total, unit="orders"):
    ...
    for i, (name, n) in enumerate(items):
        share = n / total * 100
        col = PALETTE[i % len(PALETTE)]
        segs += (f"<rect class='seg' x='{x:.2f}' y='0' width='{share:.2f}' height='30' fill='{col}'>"
                 f"<title>{name}: {n} {unit} · {share:.0f}%</title></rect>")
        legend += (f"<div class='lg'><span class='dot' style='background:{col}'></span>"
                   f"{name}<span class='lg-v'>{n} · {share:.0f}%</span></div>")
        x += share
    svg = (f"<svg viewBox='0 0 100 30' preserveAspectRatio='none' "
...
def hbars(items, w=780, color=NAVY, fmt=money, total=None):
    ...
    for i, (name, v) in enumerate(items):
        ...
        out += (f"<text x='{lab_w-10}' y='{y+rh*0.68:.0f}' text-anchor='end' class='lbl'>{name}</text>"
...
def build(week_end=WEEK_END):
    ...
    hist = [week_metrics(df, week_end - pd.Timedelta(days=7 * i))
            for i in range(1, N_TREND_WEEKS + 1)]
    avg = {k: sum(h[k] for h in hist) / len(hist)
           for k in ("gmv", "orders", "aov", "cancel_rate", "return_rate", "delivery")}
    series = {k: [h[k] for h in reversed(hist)] + [cur[k]]
              for k in ("gmv", "orders", "aov", "cancel_rate", "return_rate", "delivery")}

    # streak of consecutive WoW GMV declines ending this week
    g = series["gmv"]; streak = 0
    ...
    canc_cty = canc.groupby("country").agg(n=("order_id", "count"), val=("total", "sum")) \
                   .sort_values("n", ascending=False)
    concentrated = len(canc_cty) and canc_cty.n.iloc[0] / max(cur["cancelled"], 1) >= 0.5
    filed = cur["filed"]
    reasons = filed.return_reason.value_counts()
    bucket_of = {r: b for b, rs in RETURN_BUCKETS.items() for r in rs}
    buckets = filed.return_reason.map(lambda r: bucket_of.get(r, "Other")).value_counts()
    ...
    status_line = " · ".join(f"{n} {s.replace('_',' ')}" for s, n in filed.return_status.value_counts().items()) or "—"
    top5 = cur["nc"].groupby("product").total.sum().sort_values(ascending=False).head(5)
    repeat = [p for p, n in filed["product"].value_counts().items() if n >= 2 and p in top5.index]

    open_o = df[df.order_status.isin(["shipped", "in_transit"]) & (df.order_date <= week_end)]
    ...
    if cur["cancel_rate"] > TH_CANCEL_PCT:
        chips.append(("warn", f"⚠ Cancellations {cur['cancel_rate']:.1f}% &gt; {TH_CANCEL_PCT:.0f}%"))
    else:
        chips.append(("ok", f"Cancellations {cur['cancel_rate']:.1f}% within threshold"))
    if cur["return_rate"] > TH_RETURN_PCT:
        chips.append(("warn", f"⚠ Returns {cur['return_rate']:.1f}% &gt; {TH_RETURN_PCT:.0f}% "
                              f"({controllable} of {cur['filed_n']} company-controllable)"))
    else:
        chips.append(("ok", f"Returns {cur['return_rate']:.1f}% within threshold"))
    if len(aged):
        chips.append(("warn", f"⚠ {len(aged)} orders in transit &gt; {TH_TRANSIT_DAYS} days (oldest {oldest}d)"))
    if len(proc_aged):
        chips.append(("warn", f"⚠ {len(proc_aged)} in processing &gt; {TH_PROC_DAYS} days"))

    # executive summary (rule-based, max 3 lines)
    wow_g = pct(cur["gmv"], prev["gmv"])
    s1 = ("<b>Revenue</b> — " + f"{money(cur['gmv'])}, {'up' if wow_g>0 else 'down'} "
          f"{abs(wow_g):.1f}% vs previous week"
          + (f", {streak}th consecutive weekly decline" if streak >= 2 else
             f", first increase after {prev_streak_text(g)}" if wow_g > 0 and g[-3] > g[-2] else "")
          + f" ({delta_word(pct(cur['gmv'], avg['gmv']))} vs the 4-week average).")
    s2 = ("<b>Cancellations</b> — " + f"{cur['cancel_rate']:.1f}% ({cur['cancelled']} of "
          f"{cur['placed']} orders placed, {money(canc.total.sum())})"
          + (f", {int(canc_cty.n.iloc[0])} from {canc_cty.index[0]}." if concentrated else
             ", no country or payment concentration."))
    s3 = ("<b>Returns</b> — " + f"{cur['filed_n']} requests filed ({cur['return_rate']:.1f}% of orders), "
    ...
    cards = "".join([
        kpi("sales", "Revenue (GMV)", "Σ order total, week's non-cancelled orders", money(cur["gmv"]),
            delta_calc(cur["gmv"], prev["gmv"]),
            delta_calc(cur["gmv"], avg["gmv"], label="vs 4-wk avg", size=11),
            spark(series["gmv"])),
        kpi("sales", "Orders", "count of non-cancelled orders placed", cur["orders"],
            delta_calc(cur["orders"], prev["orders"], fmt=fm_int),
            delta_calc(cur["orders"], avg["orders"], fmt=fm_int, label="vs 4-wk avg", size=11),
            spark(series["orders"])),
        kpi("products", "Avg. order value", "revenue ÷ orders", f"${cur['aov']:,.0f}",
            delta_calc(cur["aov"], prev["aov"]),
            delta_calc(cur["aov"], avg["aov"], label="vs 4-wk avg", size=11),
            spark(series["aov"])),
        kpi("risk", "Return rate", "requests filed this week ÷ orders", f"{cur['return_rate']:.1f}%",
            delta_calc(cur["return_rate"], prev["return_rate"], invert=True, fmt=fm_pct),
            delta_calc(cur["return_rate"], avg["return_rate"], invert=True, fmt=fm_pct,
                       label="vs 4-wk avg", size=11),
            spark(series["return_rate"])),
        kpi("risk", "Cancellation rate", "cancelled ÷ all orders placed", f"{cur['cancel_rate']:.1f}%",
            delta_calc(cur["cancel_rate"], prev["cancel_rate"], invert=True, fmt=fm_pct),
            delta_calc(cur["cancel_rate"], avg["cancel_rate"], invert=True, fmt=fm_pct,
                       label="vs 4-wk avg", size=11),
            spark(series["cancel_rate"])),
        kpi("ops", "Avg. delivery time", "avg days to deliver, delivered orders", f"{cur['delivery']:.1f} d",
            delta_calc(cur["delivery"], prev["delivery"], invert=True, fmt=fm_day),
            delta_calc(cur["delivery"], avg["delivery"], invert=True, fmt=fm_day,
                       label="vs 4-wk avg", size=11),
    ...
    daily_svg = vbars_grouped([d.strftime("%a %d") for d in days],
    ...
    tab_sales = (f"<h3>Daily sales (USD) — vs previous week</h3>{daily_svg}"
                 f"<h3>Revenue by category</h3>{hbars(list(cats.items()), total=cur['gmv'])}"
                 f"<h3>Sales by country</h3>"
                 + donut(list(cty.items()), cur["gmv"])
                 + details_table("View table (verification)",
                       table(["Country", "Revenue", "Share"],
                             [[c, money(v), f"{v/cur['gmv']*100:.0f}%"] for c, v in cty.items()],
                             foot=["Total", money(cur["gmv"]), "100%"]))
                 + "<h3>Payment methods</h3>"
                 + stacked_bar([(m.replace('_',' ').capitalize(), int(n)) for m, n in pm.items()],
                               cur["orders"])
                 + details_table("View table (verification)",
                       table(["Method", "Orders", "Share"],
                             [[m.replace('_',' ').capitalize(), int(n), f"{n/cur['orders']*100:.0f}%"]
                              for m, n in pm.items()],
                             foot=["Total", cur["orders"], "100%"])))

    # ---- tab: products ----
    tab_products = (f"<h3>Top 5 products of the week (USD)</h3>{hbars(list(top5.items()), color=WARN)}"
    ...
    # ---- tab: risk ----
    canc_tbl = table(["Country", "Cancelled", "Value"],
                     [[c, int(r.n), money(r.val)] for c, r in canc_cty.iterrows()],
                     foot=["Total", cur["cancelled"], money(canc.total.sum())]) if len(canc_cty) else \
               "<p class='note'>No cancellations this week.</p>"
    reasons_str = " · ".join(f"{r.replace('_',' ').capitalize()} ({n})" for r, n in reasons.items()) or "—"
    repeat_html = (f"<div class='callout'>⚠ <b>{repeat[0]}</b> is a top-5 seller and appears "
                   f"{int(filed['product'].value_counts()[repeat[0]])} times among return requests. "
                   f"Recommended: quality / listing-accuracy check.</div>") if repeat else ""
    tab_risk = (f"<h3>Cancellations</h3>"
                f"<p class='note'>{cur['cancelled']} of {cur['placed']} orders placed ({cur['cancel_rate']:.1f}%), "
                f"{money(canc.total.sum())} lost"
                + (f", concentrated in {canc_cty.index[0]}." if concentrated else ", no concentration by country or payment method.")
                + f"</p>{canc_tbl}"
                f"<h3>Returns</h3>"
    ...
    _topics_map = {"search_policies": "Internal policies",
                   "late_return_requests": "Returns", "returns_report": "Returns",
                   "top_products": "Sales & products", "sales_breakdown": "Sales & products",
                   "sales_overview": "Sales & products", "order_lookup": "Order lookup",
                   "generate_chart": "Reports & charts"}
    _rows = ([_json.loads(l) for l in open(QUESTIONS_LOG, encoding="utf-8")]
             if QUESTIONS_LOG.exists() else [])
    ...
    try:
        ...
        if _o.getenv("LANGSMITH_API_KEY"):
            ...
            _c = _LSC()
            _since = _dt.now(_tz.utc) - _td(days=7)
            _proj = _o.getenv("LANGSMITH_PROJECT", "latram-support-agent")
            _tbt: dict = {}
            for _r in _c.list_runs(project_name=_proj, run_type="tool",
                                   start_time=_since):
                _tbt.setdefault(str(_r.trace_id), []).append(_r.name)
            for _r in _c.list_runs(project_name=_proj, is_root=True,
                                   start_time=_since):
                _m = _re.search(r"content['\"]:\s*['\"](.+?)(?:\\n\\n\[System note|['\"])",
                                str(_r.inputs))
                if _m:
                    _rows.append({"question": _m.group(1)[:160],
                                  "tools": _tbt.get(str(_r.trace_id), [])})
    except Exception:
        pass
    if _rows:
        _top5 = [q for q, _ in _Counter(r["question"] for r in _rows).most_common(5)]
        _tt = _Counter(_topics_map.get(t, "Other")
                       for r in _rows for t in r.get("tools", []))
        _tip = ""
        if _tt:
            _tname, _tn = _tt.most_common(1)[0]
            _tip = (f"<div class='callout'>{_tn/sum(_tt.values()):.0%} of questions focus on "
                    f"<b>{_tname}</b> — suggested topic for the next internal training.</div>")
        tab_team = ("<h3>Most frequent team questions</h3><ol class='qs'>"
                    + "".join(f"<li>{q}</li>" for q in _top5) + "</ol>" + _tip
                    + f"<p class='note'>Source: {len(_rows)} logged interactions with "
                      f"{AGENT_NAME} (local log + LangSmith, last 7 days).</p>")
    else:
        tab_team = ("<h3>Most frequent team questions</h3>"
                    "<p class='note'>No interactions logged yet this period.</p>")
    ...
    method_rows = [
        ...
        ["Avg. delivery time",
         ...
         f"= {cur['delivery']:.1f} d"],
        ["vs prev. week", "(this week − previous week) ÷ previous week",
         f"e.g. revenue: ({money(cur['gmv'])} − {money(prev['gmv'])}) ÷ {money(prev['gmv'])} "
         f"= {wow_g:+.1f}%"],
        ["vs 4-wk avg", "(this week − average of the 4 prior weeks) ÷ that average",
         f"prior weekly revenue: {wk4} → avg {money(avg['gmv'])}"],
        ["Return buckets",
         " · ".join(f"<b>{b}</b>: {', '.join(rs)}" for b, rs in RETURN_BUCKETS.items()),
         f"{bucket_line} → {controllable} of {cur['filed_n']} company-controllable"],
        ["Sparkline", "the metric's value for the last 5 closed weeks (4 prior + current)", "—"],
    ]
    y, m0 = week_end.year, cur["start"].month
    excel_check = (f"Excel check for revenue: <code>=SUMIFS(total, order_date, "
    ...
    tab_method = ("<h3>How every number is calculated</h3>"
...
<script>{JS}</script></body></html>"""
    ...
    print(f"GMV {money(cur['gmv'])} ({wow_g:+.1f}% vs prev wk) | orders {cur['orders']} | AOV ${cur['aov']:.2f} | "
          f"cancel {cur['cancel_rate']:.1f}% | return {cur['return_rate']:.1f}% | deliv {cur['delivery']:.1f}d")
    print(f"4wk avg GMV {money(avg['gmv'])} | chips: {[t for _, t in chips]} | repeat: {repeat} | streak: {streak}")

def delta_word(d): return f"{'up' if d > 0 else 'down'} {abs(d):.1f}%"
...
def main() -> None:
    ...
    if not (to and user and pwd):
        raise EnvironmentError("Set REPORT_TO, SMTP_USER, SMTP_APP_PASSWORD in .env")
    url = _os.getenv("LARA_URL", "").strip()
    cta = (f"<p style='margin:22px 0'><a href='{url}' style='background:{NAVY};color:#fff;"
           f"padding:11px 24px;border-radius:8px;text-decoration:none;font-weight:700'>"
           f"Ask {AGENT_NAME}</a></p>" if url else "")
    period = WEEK_END.strftime("%d %b %Y")
    body = (f"<html><body style='font-family:Arial;color:#333'>"
            ...
            f"<b>{period}</b>) is attached. Open the HTML file in a browser for the "
            f"interactive version: tabs, hover formulas and full methodology.</p>{cta}"
    ...
    msg["Subject"] = f"{AGENT_NAME} · Weekly report — {BRAND_NAME} — week ending {period}"
    msg["From"] = f"{AGENT_NAME} · {BRAND_NAME} <{user}>"
    msg["To"] = to
    msg.attach(MIMEText(body, "html", "utf-8"))
    logo = MIMEImage(open(LOGO_PNG, "rb").read()); logo.add_header("Content-ID", "<logo>")
    msg.attach(logo)
    ...
    print(f"Reporte enviado a {to} (HTML interactivo adjunto)")
...
def report_summary(week_ending: str = "") -> str:
    ...
    Optional `week_ending` (ISO date) retrieves a PAST report's week — any
    date is snapped to its closed Mon-Sun week; empty = latest closed week.
    Same functions that generate the report — single source of truth."""
    df = load()
    end = pd.Timestamp(week_ending) if week_ending else WEEK_END
    end = (end - pd.Timedelta(days=(end.weekday() + 1) % 7)).normalize()
    if end > WEEK_END:
        end = WEEK_END
