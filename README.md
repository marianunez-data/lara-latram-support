...
| Latency | ~12s median end-to-end per question, ~34s on tool-heavy ones | measured: `latency_s` in the local log. LangSmith's 2.55s P50 counts individual runs incl. sub-second tool calls, not whole questions |
...
| Tokens | ~4,700 median per question (range 3.6k–16.3k) | measured: provider `usage_metadata` in the local log; agrees with LangSmith (190,244 tokens / 35 traces ≈ 5.4k) |
| Cost per question | $0.0007–$0.012 depending on which provider answers | measured: $0.13 across 35 traced questions |
| Daily capacity | bounded by free-tier provider quotas; the fallback absorbs bursts | provider quotas — check current docs before quoting |
| Interaction history | not persisted in the deployed app: the container disk resets, so questions asked there survive only as 14-day traces | Hugging Face free-tier behavior |
...
**Costs.** The current stack runs on free tiers. Measured spend was $0.13
across 35 traced questions — extrapolating, roughly **$11/month at 100
questions/day**. The per-question cost varies by an order of magnitude
depending on which provider answers, which is the dual-provider design
showing up in the bill. Spend scales with traffic, not with data volume:
adding policy PDFs or rows to the sheet changes disk and RAM, not the bill.
